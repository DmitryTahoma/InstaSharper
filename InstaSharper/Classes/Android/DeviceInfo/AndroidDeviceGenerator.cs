...
namespace InstaSharper.Classes.Android.DeviceInfo
{
    ...
    {
        public static bool UseTrustDevices { set; get; } = false;
        ...
        public static readonly Dictionary<string, AndroidDevice> AndroidAndroidDeviceSets = new Dictionary<string, AndroidDevice>
        {
            {
                "lg-optimus-g",
                new AndroidDevice
        ...
        public static List<AndroidDevice> TrustDevices { private set; get; } = new List<AndroidDevice>();

        public static AndroidDevice GetRandomAndroidDevice()
        {
            var random = new Random(DateTime.Now.Millisecond);
            if (UseTrustDevices && TrustDevices.Count > 0)
            {
                int randomDeviceIndex = random.Next(0, TrustDevices.Count);
                return TrustDevices[randomDeviceIndex];
            }
            else
            {
                var randomDeviceIndex = random.Next(0, DevicesNames.Count);
                var randomDeviceName = DevicesNames[randomDeviceIndex];
        ...
        public static void AddTrustDevice(AndroidDevice device) => TrustDevices.Add(device);

        public static void RemoveTrustDevice(AndroidDevice device) => TrustDevices.Remove(device);
    }
}
